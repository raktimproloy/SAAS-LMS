      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center bg-card/40 backdrop-blur-3xl p-1.5 rounded-2xl border border-white/10 shadow-sm overflow-x-auto w-full sm:w-auto">
          {['all', 'online', 'offline'].map((f) => (
            <button key={f} className={`px-5 py-2 rounded-xl transition-all font-bold capitalize whitespace-nowrap flex-1 sm:flex-none ${f === 'all' ? 'bg-primary text-primary-foreground shadow-lg' : 'text-muted-foreground hover:text-white hover:bg-white/5'}`}>
              {f}
            </button>
          ))}
        </div>
        <div className="flex items-center bg-card/40 backdrop-blur-3xl p-1.5 rounded-2xl border border-white/10 shadow-sm shrink-0">
          ...
          <button className="p-2.5 rounded-xl transition-all flex items-center gap-2 font-bold text-muted-foreground hover:text-white hover:bg-white/5">
