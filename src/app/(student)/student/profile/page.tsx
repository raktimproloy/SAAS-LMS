...
export default function StudentProfile() {
  ...
  return (
    <div className="space-y-8 w-full max-w-[1920px] mx-auto pb-12">
      ...
      {/* Top Banner Section */}
      <div 
        className="relative bg-card/90 dark:bg-card/40 backdrop-blur-3xl p-8 md:p-12 rounded-[2.5rem] shadow-2xl flex flex-col md:flex-row items-center md:items-start gap-8 border border-border group"
        data-aos="fade-down"
      >
        ...
        <Button
          ...
          className="absolute top-6 right-6 z-20 gap-2 rounded-xl bg-background/40 backdrop-blur-md border-border hover:bg-background/60"
        >
        ...
        <div className="relative z-10 shrink-0">
          <div
            className="w-32 h-32 md:w-40 md:h-40 rounded-[2rem] overflow-hidden border-2 border-border shadow-[0_0_30px_rgba(var(--primary),0.3)] bg-gradient-to-br from-primary/40 to-cyan-500/40 flex items-center justify-center relative group/avatar backdrop-blur-md cursor-pointer"
            onClick={openEditDialog}
          >
            {student.photo ? (
              <img src={student.photo} alt={student.name} className="w-full h-full object-cover" />
            ) : (
              <span className="text-5xl font-black text-foreground mix-blend-overlay opacity-80">{student.name.substring(0, 2).toUpperCase()}</span>
            )}
        ...
        <div className="relative z-10 flex-1 text-center md:text-left space-y-4 w-full">
          ...
          <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 pt-2 text-sm font-medium text-muted-foreground">
            {student.phone && (
              <div className="flex items-center gap-2 bg-background/40 px-3 py-1.5 rounded-xl border border-border/60 backdrop-blur-md">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        {/* Personal Info Card */}
        <div 
          className="bg-card/90 dark:bg-card/40 backdrop-blur-2xl border border-border rounded-3xl p-8 shadow-xl relative overflow-hidden group hover:bg-card/90 dark:bg-card/60 transition-colors"
          data-aos="fade-up"
        >
          ...
          <h2 className="text-xl font-bold mb-6 flex items-center gap-3 text-foreground relative z-10">
          ...
          <div className="space-y-6 relative z-10">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 border-b border-border/60">
            ...
            <div className="flex flex-col items-center gap-3">
              <div className="w-24 h-24 rounded-2xl overflow-hidden border-2 border-border bg-gradient-to-br from-primary/40 to-cyan-500/40 flex items-center justify-center relative">
                {editPhoto ? (
                  ...
                  <span className="text-2xl font-black text-foreground mix-blend-overlay opacity-80">{editName.substring(0, 2).toUpperCase()}</span>
                )}
                {isUploadingPhoto && (
