...
export function VideoCourseSection() {
  ...
  return (
    <section className="py-24 overflow-hidden relative" style={{ backgroundColor: '#131124' }}>
      
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 mb-12">
        <div className="text-center" data-aos="fade-up">
          <span className="inline-block py-1 px-3 rounded-full bg-indigo-500/10 text-indigo-400 font-semibold text-sm uppercase tracking-widest border border-indigo-500/20 mb-4">
            ভিডিও কোর্স
          </span>
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-white mb-6">সকল রেকর্ডেড ক্লাস</h2>
        ...
        <div className="max-w-4xl mx-auto px-4 mt-2 sm:mt-6 h-auto sm:h-[200px] relative">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="w-full bg-white/5 border border-white/10 rounded-3xl p-6 sm:p-8 backdrop-blur-xl shadow-2xl relative overflow-hidden"
            >
              ...
              <div className="flex flex-col sm:flex-row items-start gap-6 sm:gap-10 relative z-10">
                 ...
                 <div className="flex-1 w-full">
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <div className="hidden sm:flex items-center gap-1.5 px-3 py-1 bg-white/10 rounded-full text-white/80 text-xs font-semibold border border-white/10">
                        <Users className="w-3.5 h-3.5 text-indigo-400" />
                        {activeCourse.students}
                      </div>
                    </div>
                    
                    <div className="flex sm:hidden items-center gap-1.5 px-3 py-1 bg-white/10 rounded-full text-white/80 text-xs font-semibold border border-white/10 w-fit mb-4">
                        <Users className="w-3.5 h-3.5 text-indigo-400" />
                        {activeCourse.students}
                    </div>

                    <p className="text-white/60 text-sm sm:text-base leading-relaxed pr-0 sm:pr-8">
                      {activeCourse.description}
                    </p>
                 </div>

                 {/* Right side: Stats / Tags */}
                 <div className="w-full sm:w-auto shrink-0 grid grid-cols-3 sm:grid-cols-1 gap-4 sm:gap-6 border-t sm:border-t-0 sm:border-l border-white/10 pt-6 sm:pt-0 sm:pl-10">
                    {activeCourse.stats.map((stat, idx) => {
                      ...
                      return (
                        <div key={idx} className="flex flex-col sm:flex-row items-center sm:items-start gap-3 text-center sm:text-left group/stat cursor-default">
                          <div className="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center shrink-0 group-hover/stat:bg-white/10 transition-colors">
                            <Icon className="w-5 h-5 text-indigo-300 group-hover/stat:text-indigo-200 transition-colors" />
                          </div>
                          <div>
                            <div className="text-lg sm:text-xl font-bold text-white">{stat.label}</div>
