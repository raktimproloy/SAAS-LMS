...
import { Dna, MapPin, Phone, Mail, MessageCircle } from "lucide-react";
...
export function Footer() {
  return (
    <footer className="relative bg-slate-950 dark:bg-slate-950 text-slate-300 overflow-hidden">
      {/* Decorative top glow with animation */}
      <motion.div 
        animate={{ opacity: [0.3, 1, 0.3], scaleX: [0.8, 1.2, 0.8] }}
        transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
        className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-[2px] bg-gradient-to-r from-transparent via-indigo-500 to-transparent" 
      />
      <motion.div 
        animate={{ opacity: [0.2, 0.6, 0.2], scale: [0.9, 1.1, 0.9] }}
        transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
        className="absolute top-0 left-1/2 -translate-x-1/2 w-[300px] h-[80px] bg-indigo-600/20 blur-3xl rounded-full" 
      />
          ...
          >
            <Link href="/" className="flex items-center gap-2.5 group w-fit">
              ...
              <span className="font-bold text-xl text-white">
                Doctor<span className="text-sky-300">Biology</span>
              </span>
            </Link>
              ...
              {socialLinks.map((s) => {
                ...
                return (
                  <a
                    ...
                    aria-label={s.label}
                    className={cn(
                      "h-9 w-9 rounded-xl bg-white/8 border border-white/10 flex items-center justify-center transition-all duration-200 hover:border-white/25 hover:bg-white/15 hover:scale-110 text-white/70",
                      s.color
                    )}
                  >
            ...
            <ul className="space-y-2.5">
              {footerLinks.map((link) => (
                <li key={link.label}>
                  <Link
                    href={link.href}
                    className="text-sm text-slate-400 hover:text-white transition-colors hover:translate-x-1 inline-flex items-center gap-1.5 group"
                  >
          ...
          >
            ...
            <ul className="space-y-3">
              ...
              <li className="flex items-center gap-3 text-sm text-slate-400">
                ...
                <a href="mailto:info@doctorbiology.com" className="hover:text-white transition-colors">
